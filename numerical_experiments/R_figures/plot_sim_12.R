...
# data output
plt_fpath <- paste0(our_dir, "/onesample_ttest_power.png")

eff_tar_levels  <- c("0.3", "0.4",  "observed")
...
name_labels     <- c("upstrap", "power.t.test")
N_obs_tmp <- 50
...
for (i in 1 : length(eff_tar_levels)){ # i <- 2
  ...
  x_axis_labels <- sapply(x_axis_breaks, function(N_tar){N_tar}) 
  plt_df <- 
    dat_agg %>% 
  ...
  if (i == 1){
    plt <- plt + labs(title = "Simulation problem 1") + 
...
# data output
plt_fpath <- paste0(our_dir, "/twosample_ttest_power.png")
...
for (i in 1 : length(eff_tar_levels)){ # i <- 2
  ...
  if (i == 1){
    plt <- plt + labs(title = "Simulation problem 2") + 
      theme(plot.title = element_text(size = 14, face = "bold"))
  }
  plt_list[[length(plt_list) + 1]] <- plt
}
