...
dat_agg <- 
  dat %>%
  group_by(N_tar, N_obs, name, eff_tru, eff_tar, sigma2, tau2) %>%
  summarise(
    cnt = sum(!is.na(value)),
    value_mean = mean(value),
    value_sd = sd(value),
...
dat_truepower_agg <- 
  dat %>%
  filter(name == "true_power") %>% 
  group_by(N_tar, N_obs, name, eff_tar, sigma2, tau2) %>%
  summarise(value = mean(value)) %>% 
  pivot_wider(names_from = name, values_from = value) %>%
  ungroup() %>%
  select(-N_obs) %>%
...
dat_diff_A <- 
  ...
  filter(name != "true_power") %>% 
  pivot_wider(names_from = name, values_from = value) %>%
  left_join(dat_truepower_agg, by = c("N_tar", "eff_tar", "sigma2", "tau2")) %>%
  mutate(
    pe_ups_true =  100 * (upstrap_power - true_power)/true_power
  ) %>%
  select(-c(upstrap_power, true_power)) %>%
...
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# LMM: vary N_obs and error(s) sd (2)

fdir_raw  <- paste0(here::here(), "/numerical_experiments/results_CL/2021-12-03-lmm_testcoef_vary_nobs_sd_2_raw")
fpath_out <- paste0(here::here(), "/numerical_experiments/results_CL_shared/2021-12-03-lmm_testcoef_vary_nobs_sd_2_agg.rds")
