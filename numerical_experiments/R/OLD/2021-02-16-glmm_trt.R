...
# define results dir
out_dir_raw <- paste0(here::here(), "/numerical_experiments/results_CL/2021-02-16-glmm_trt_raw")
...
coef_x1 <- 0.6 # change from lmm to glm 
tau2    <- 1
sigma2  <- 1
...
for (innerloop_idx in 1 : innerloop_N){ # innerloop_idx <- 1
  ...
  mat_out_tmp$out_gold_GLMM  <- NA
  mat_out_tmp$out_gold_GEE  <- NA
  ...
  for (N1_grid_idx in 1 : N1_grid_l){ # N1_grid_idx <- 10
    ...
    mat_out_tmp$out_gold_GLMM[N1_grid_idx] <- (fit_GLMM_coefest_pval < 0.05) * 1
    # GEE
    ...
    mat_out_tmp$out_gold_GEE[N1_grid_idx] <- (fit_GEE_coefest_pval < 0.05) * 1
  }
  ...
  mat_out_boot_GLMM  <- matrix(NA, nrow = N1_grid_l, ncol = B_boot)
  ...
  for (B_boot_idx in 1:B_boot){ # B_boot_idx <- 2
    ...
    for (N1_grid_idx in 1:N1_grid_l){ # N1_grid_idx <- 1
      ...
      fit_GLMM_s <- summary(fit_GLMM)
      fit_GLMM_coefest_pval  <- fit_GLMM_s$coefficients[2, 4]
      mat_out_boot_GLMM[N1_grid_idx, B_boot_idx] <- (fit_GLMM_coefest_pval < 0.05)
      # GEE
      fit_GEE_formula <- formula(y ~ x1)
      fit_GEE <- geeglm(formula = fit_GEE_formula, family = binomial(link = "logit"), 
                        data = dat_sub, id = dat_sub$subjid, corstr = "exchangeable")
      fit_GEE_s <- summary(fit_GEE)
      fit_GEE_coefest_pval  <- fit_GEE_s$coefficients[2, 4]
      mat_out_boot_GEE[N1_grid_idx, B_boot_idx] <- (fit_GEE_coefest_pval < 0.05)
    }
  }
  # add results to mat_out
  mat_out_tmp$out_boot_GLMM <- rowMeans(mat_out_boot_GLMM, na.rm = TRUE)
