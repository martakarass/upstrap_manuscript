...
#' Rnosave 2021-02-22-glmm_trt_aggresults.R -N JB_glmm_trt_agg
...
# dir to data saves
res_fdir_raw  <- paste0(here::here(), "/numerical_experiments/results_CL/2021-02-17-glmm_trt_raw")
res_fdir_agg  <- paste0(here::here(), "/numerical_experiments/results_CL_shared/2021-02-17-glmm_trt_agg")
dir.create(path = res_fdir_agg)
message(paste0("dir.exists(path = res_fdir_agg): ", dir.exists(path = res_fdir_agg)))

# experiment parameters (relevant subset; see /R/2021-02-17-glmm_trt.R)
...
ylab_vec <- c("out_gold_GLMM", "out_gold_GEE", "out_boot_GLMM", "out_boot_GEE")
out_df_all <- data.frame()

t1 <- Sys.time()
for (i in 1 : length(ylab_vec)){ # i <- 2
...
# save the aggregated data 
head(out_df_all) 
saveRDS(out_df_all, paste0(res_fdir_agg, "/glmm_trt_samplesize_est_bootCI.rds"))
rm(out_df_all)
