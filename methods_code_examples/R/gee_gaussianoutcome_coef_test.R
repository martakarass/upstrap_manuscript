...
library(geepack)
fit  <- geeglm(formula = y ~ x1 + x2, family = gaussian(link = "identity"), 
               data = dat, id = dat$subjid, corstr = "exchangeable")
coef(fit)["x1"]
# 0.6890758 
...
for (rr in 1 : R_boot){
  ...
  fit_rr  <- geeglm(formula = y ~ x1 + x2, family = gaussian(link = "identity"), 
                    data = dat_rr, id = dat_rr$subjid, corstr = "exchangeable")
...
dat_upd <- dat
dat_upd$y <- dat_upd$y + (1.2 - coef(fit)["x1"]) * dat_upd$x1
out <- rep(NA, R_boot)
for (rr in 1 : R_boot){
  print(rr)
